...
void *enc_service(void *args) {
   ...
   // if e is not present then the encryption client signative is not present
   if ((client = strchr(delim, 'e')) == NULL) {
      strcpy(sendBuffer, "ERROR attempted connection with otp_enc_d$"); // trailing $ signifies error message
      charsRead = send(establishedConnectionFD, sendBuffer, sizeof(sendBuffer), 0); // error message back
   } else {
      int textLength = strlen(text);
      int i, temp, keyval;
      for (i = 0; i < textLength; i++) {
         temp = (text[i] - 64); // sub by 64 so that letters are valued 1 to 26
         keyval = (key[i] - 64);
         if (temp < 1) { // anything less than 1 was 32=' ' which is 27 in our system
            temp = 27;
         }
         if (keyval < 1) {
            keyval = 27;
         ...
         temp += keyval; // apply encryption values
         if (temp > 27) { // if value outside of our char range
            if (temp == 54) { // these were our spaces + spaces = spaces
               temp = 27;
            } else { // else we ger remainder
               temp = temp % 27;
            }
         }

         if (temp == 27) { // reset our value of spaces to ascii value
            temp = 32;
         } else { // else add to return to ascii letter value
            temp += 64;
         }
         sendBuffer[i] = (char) temp;
      }
      sendBuffer[strlen(sendBuffer)] = '!'; // apply ! at end for message terminating char

      // Send a Success message back to the client
      charsRead = send(establishedConnectionFD, sendBuffer, sizeof(sendBuffer), 0); // Send success back
      if (charsRead < 0) { // if error send then manage memory and exit thread
         (*active_threads)--;
         pack->active_threads = NULL;
         free(pack);
         close(establishedConnectionFD);
      ...
      close(establishedConnectionFD); // Close the existing socket which is connected to the client
       ...
       // if we made it to the end then decrement thread count and clean up/exit with detach
      (*active_threads)--;
      pack->active_threads = NULL;
